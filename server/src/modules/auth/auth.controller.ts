...
export const register = async (req: Request, res: Response) => {
  try {
    ...
    if (existingPaidUser) {
      const conflictField = existingPaidUser.email === email ? 'email' : 'contact';
    ...
    if (existingUnpaidUser) {
      ...
      const paidUserWithSameDetails = await User.findOne({
        $or: [
          { email: existingUnpaidUser.email !== email ? email : null, paymentStatus: 'completed' },
          { contact: existingUnpaidUser.contact !== contact ? contact : null, paymentStatus: 'completed' }
        ],
        _id: { $ne: existingUnpaidUser._id }
      });

      if (paidUserWithSameDetails) {
        const conflictField = paidUserWithSameDetails.email === email ? 'email' : 'contact';
        return res.status(httpStatus.CONFLICT).json({
          message: `User with this ${conflictField} already exists and has completed payment`
        });
      }
      ...
      if (existingUnpaidUser.contact !== contact) existingUnpaidUser.contact = contact;
